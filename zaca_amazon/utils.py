...
from odoo.addons.zaca_amazon import const as zconst
...
def make_sp_api_request(account, operation, path_parameter='', payload=None, method='GET'):
    ...
    path = zconst.API_OPERATIONS_MAPPING[operation]['url_path'].format(param=path_parameter)
    ...
    if zconst.API_OPERATIONS_MAPPING[operation]['restricted_resource_path'] is None:  # No RDT is required
...
def refresh_restricted_data_token(account):
    ...
    if datetime.utcnow() > account.restricted_data_token_expiry - timedelta(minutes=5):
        all_restricted_operations = [
            k for k, map in zconst.API_OPERATIONS_MAPPING.items() if map['restricted_resource_path']
        ]
        OPERATIONS_MAPPING = zconst.API_OPERATIONS_MAPPING
        payload = {
            'restrictedResources': [{
                'method': 'GET',
